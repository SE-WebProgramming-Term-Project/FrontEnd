...
function AdminPageAnalysis() {
  ...
  const [pizzalabels, setPizzalabels] = useState([]);
    ...
    const fetchPizzaNames = async () => {
      try {
        ...
        const responses = await Promise.all(
          pizzaNames.map((_id) =>
            axios.get("http://localhost:5000/pizza/view", { params: {_id} })
          )
        );
        const fetchedPizzaLabels = responses.map(
          (response) => response.data.pizzaData.title
        );
        setPizzalabels(fetchedPizzaLabels);
      } catch (error) {
        console.log(error);
      }
    };
